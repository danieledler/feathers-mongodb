...
class Service {
  /**
   * A feathers-mongodb adapter
   * @param {Object} options Mongo adapter option:
   *   Model {mongodb client collection}: mongodb collection instance
   *   id {string}: id field (default '_id')
   *   paginate {object}: pagination object (default empty)
   *   useStringId {bool}: use stringified ObjectID (default false)
   */
  constructor (options) {
    if (!options) {
    ...
    this.useStringId = options.useStringId || false;
  }
  ...
  _objectifyId (id) {
    if (!this.useStringId && this.id === '_id' && ObjectID.isValid(id)) {
  ...
  create (data, params) {
    const setId = item => {
      ...
      // Generate a MongoId if we use a custom id or string id
      if ((this.id !== '_id' || this.useStringId) && typeof entry[this.id] === 'undefined') {
